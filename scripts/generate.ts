...
function tsTypeToTsp(typeName: string): string {
  ...
  // จัดการเกี่ยวกับ Union Types เบื้องต้น เช่น Date | null -> utcDateTime | null
  if (t.includes('|')) {
    return t
      .split('|')
...
function buildModelBlock(
  ...
  // นิยามฟิลด์ที่เป็นส่วนกลาง (Base Schema) ที่เราต้องการทำ Spread
  const BASE_FIELDS = ['id', 'createdAt', 'updatedAt', 'deletedAt'];

  // แยก Props ออกเป็นฟิลด์ทั่วไป (Domain) และฟิลด์เบส (Base)
  const hasBaseFields = props.some((p) => BASE_FIELDS.includes(p.name));
  const domainProps = props.filter((p) => !BASE_FIELDS.includes(p.name));

  // 1. ถ้าตรวจเจอว่าโมเดลนี้มีพวกฟิลด์เบส ให้ใส่ ...BaseEntity เข้าไปก่อนเลย
  if (hasBaseFields) {
    lines.push(`    ...BaseEntity;`);
    if (domainProps.length > 0) lines.push(''); // เติมบรรทัดว่างให้สวยงาม
  }

  // 2. เขียน Attributes อื่น ๆ ของ Schema ตามมา
...
const entitiesGlob = path.join(root, 'src/domains/entities/**/*.ts');
const outPath = path.join(root, 'src/lib/spec/models/base-entities.tsp');
...
// สร้าง BaseEntity กลาง และนำมาร้อยรวมกับ blocks ทั้งหมด
