...
const ProjectDetail = ({ className, project }: ProjectDetailProps) => {
  ...
  const gridImages =
    project.images && project.images.length > 1 ? project.images.slice(1) : []
  ...
  const primaryTag =
    project.tags && project.tags.length > 0 ? project.tags[0] : 'Architecture'

  const tagsList =
    project.tags && project.tags.length > 0 ? project.tags.join(', ') : 'Design'
      ...
      <div className="container space-y-6 mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:justify-between font-medium gap-4">
          <p className="max-w-3xl leading-relaxed text-muted-foreground">
            A dynamic display of structural layout, materials design, and
        ...
        <FadeUpOnScroll delay={0.15}>
          <div className="overflow-hidden relative aspect-video w-full bg-muted">
            <Image
              src={heroImage}
              alt={project.name}
              fill
        ...
        {gridImages.length > 0 && (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:gap-6 pt-4">
            {gridImages.map((src, i) => (
              <div
                key={i}
                className="overflow-hidden relative aspect-video w-full bg-muted"
              >
                <Image
                  src={src}
